...
@AggregationFunction(value = "learn_mlp_classifier", decomposable = false)
public class LearnMlpVarcharClassifierAggregation
{
    private LearnMlpVarcharClassifierAggregation() {}
    ...
    public static void output(@AggregationState LearnState state, BlockBuilder out)
    {
        ...
        Model model = new StringClassifierAdapter(new ClassifierFeatureTransformer(new MlpClassifier(MlpUtils.parseParameters(state.getParameters().toStringUtf8())), new FeatureUnitNormalizer()));
        model.train(dataset);
