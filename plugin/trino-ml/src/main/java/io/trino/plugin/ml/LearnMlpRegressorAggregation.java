...
@AggregationFunction(value = "learn_mlp_regressor", decomposable = false)
public class LearnMlpRegressorAggregation
{
    private LearnMlpRegressorAggregation() {}
    ...
    public static void output(@AggregationState LearnState state, BlockBuilder out)
    {
        ...
        Model model = new RegressorFeatureTransformer(new MlpRegressor(MlpUtils.parseParameters(state.getParameters().toStringUtf8())), new FeatureUnitNormalizer());
        model.train(dataset);
