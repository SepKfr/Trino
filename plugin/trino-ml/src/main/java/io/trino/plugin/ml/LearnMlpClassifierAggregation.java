...
public class LearnMlpClassifierAggregation
{
    private LearnMlpClassifierAggregation() {}
    ...
    public static void output(@AggregationState LearnState state, BlockBuilder out)
    {
        ...
        Model model = new ClassifierFeatureTransformer(new MlpClassifier(MlpUtils.parseParameters(state.getParameters().toStringUtf8())), new FeatureUnitNormalizer());
        model.train(dataset);
